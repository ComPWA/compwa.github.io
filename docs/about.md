...
- {doc}`PWA Expert System <expertsystem:index>` (split into {doc}`QRules <qrules:index>`
  and {doc}`AmpForm <ampform:index>`).
...
3. It should be **easy to make the step from using the library to contributing to the
   ...
   {ref}`example pages provided by each library <about:User experience>` (accessible
...
5. As frameworks are {ref}`open source <about:Open source>` and users and developers
...
- Almost all ComPWA libraries are written in Python. This makes it easy for analysis
  ...
  make the step to become a {ref}`developer <about:Developer Experience>`.
